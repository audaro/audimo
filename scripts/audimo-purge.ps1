...
#
# ASCII-only by design: Windows PowerShell 5.1 (the default
# powershell.exe) does not read UTF-8 files without a BOM and will
# misparse non-ASCII characters in comments (em-dashes, arrows, etc.)
# as truncated multi-byte sequences, breaking the script with bogus
# "Missing closing '}'" errors. Keep this file 7-bit clean.
...
# image path is under .audimo\addons.
...
winget uninstall --silent --id app.audimo.desktop 2>&1 | Out-Null
if ($LASTEXITCODE -eq 0) {
    Done "  winget uninstall ok"
} else {
    Warn "  winget did not find app.audimo.desktop; falling back to registry scan"
    $uninstallRoots = @(
...
$dataDirs = @(
    ...
    "$env:APPDATA\audimo",
    "$env:LOCALAPPDATA\audimo",
    "$env:LOCALAPPDATA\Programs\Audimo",
    "$env:LOCALAPPDATA\app.audimo.desktop",
    "${env:ProgramFiles}\Audimo"
)
...
# We only know ffmpeg got installed because of Audimo if it came from
# our winget call (Gyan.FFmpeg). Don't touch a manually-installed
